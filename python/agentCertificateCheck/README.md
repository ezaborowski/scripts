...
* If you are unable to run the 'chmod' command against the script for any reason, please run the script with the preface of 'python3' (or 'python' depending on the version of python currently installed on the system:

```bash
python3 agentCertificateCheck.py -v mycluster -u myuser -d local
```
To validate which version of python is installed on the system:
```bash
python --version
```
